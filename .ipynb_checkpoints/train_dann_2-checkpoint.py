...
from models import ClimateRCAN_ShallowFusion_DANN
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Second Optuna DANN search for shallow-fusion RCAN HR-aux climate "
            "downscaling, using expanded high-capacity hyperparameter ranges."
        )
    )
    ...
    parser.add_argument("--num_resblk_min", type=int, default=18)
    parser.add_argument("--num_resblk_max", type=int, default=24)
    ...
    parser.add_argument("--learning_rate_min", type=float, default=2e-4)
    parser.add_argument("--learning_rate_max", type=float, default=5e-4)
    parser.add_argument("--weight_decay_min", type=float, default=1e-7)
    parser.add_argument("--weight_decay_max", type=float, default=1e-3)

    parser.add_argument("--alpha_domain_min", type=float, default=2e-5)
    parser.add_argument("--alpha_domain_max", type=float, default=1.5e-3)
    parser.add_argument("--lambda_grl_max", type=float, default=0.3)
    parser.add_argument(
...
def make_save_root(args, exp_folder_name):
    if args.save_dir is not None:
        ...
        save_root = os.path.join(
            ...
            "trained_models",
            MODEL_NAME,
            "optuna_lr_gcm_with_hr_mask_elevation_to_hr_rcm_dann_2",
        )
...
def make_model(hparams, args, input_channels=1, hr_aux_channels=2):
    return ClimateRCAN_ShallowFusion_DANN(
...
def build_config(args, hparams):
    return {
        ...
        "search_name": "dann_2_expanded_capacity_tuned_domain_search",
        "target_domain": (
...
def main():
    ...
    if args.study_name is None:
        study_name = (
            f"rcan_hr_aux_shallowfusion_dann_2_{args.rcm_var}_{args.gcm_name}_"
