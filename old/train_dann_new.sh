...
LAMBDA_GRL_MAX=0.3
# Fill this only with the previous model's normalized full-image target
# validation MSE for the same validation years. Leave empty otherwise.
BASELINE_NAME=previous_dann_same_val_metric
BASELINE_VAL_LOSS=
...
RUN_NAME="dann_new_vgg_domain_${DOMAIN_CONDITION}_${VAL_START_YEAR}_${VAL_END_YEAR}_bs${BATCH_SIZE}"
SAVE_DIR="$EXP_DIR/trained_models/$MODEL_NAME/$RUN_NAME"
BASELINE_ARGS=()
if [[ -n "$BASELINE_VAL_LOSS" ]]; then
    BASELINE_ARGS=(
