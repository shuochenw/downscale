...
from models import SRResNet_HR_Aux
...
save_root = os.path.join(
    ...
    "trained_models",
    "SRResNet_HR_Aux",
    "GCM_low_res_with_HR_mask_elevation_plain_mse",
)
...
hr_elev = torch.load(os.path.join(exp_folder_name, hr_elev_file)).float()

hr_mask = match_sample_dim(hr_mask, y.shape[0], "hr_mask")
hr_elev = match_sample_dim(hr_elev, y.shape[0], "hr_elev")
hr_mask = (hr_mask > 0.5).float()
hr_elev = torch.nan_to_num(hr_elev, nan=0.0, posinf=0.0, neginf=0.0)
...
if y.shape != hr_elev.shape:
    raise ValueError(f"Target shape {y.shape} does not match HR elevation shape {hr_elev.shape}")

assert_finite(X, "X")
assert_finite(y, "y")
assert_finite(hr_mask, "hr_mask")
assert_finite(hr_elev, "hr_elev")

print("Loaded shapes:")
print("X      :", X.shape)
print("y      :", y.shape)
print("hr_mask:", hr_mask.shape)
print("hr_elev:", hr_elev.shape)
...
hr_mask_train = hr_mask[:train_end_idx]
hr_elev_train = hr_elev[:train_end_idx]

X_val = X[val_start_idx:]
y_val = y[val_start_idx:]
hr_mask_val = hr_mask[val_start_idx:]
hr_elev_val = hr_elev[val_start_idx:]

print("Split shapes:")
print("X_train     :", X_train.shape)
print("y_train     :", y_train.shape)
print("hr_mask_train:", hr_mask_train.shape)
print("hr_elev_train:", hr_elev_train.shape)
print("X_val       :", X_val.shape)
print("y_val       :", y_val.shape)
print("hr_mask_val :", hr_mask_val.shape)
print("hr_elev_val :", hr_elev_val.shape)
...
X_train_in = X_train_n.contiguous()
X_val_in = X_val_n.contiguous()

X_train_hr_aux = torch.cat(
    [hr_mask_train, hr_elev_train],
    dim=1,
).contiguous()

X_val_hr_aux = torch.cat(
    [hr_mask_val, hr_elev_val],
    dim=1,
).contiguous()
...
# datasets
# =========================================================
training_set = TensorDataset(X_train_in, X_train_hr_aux, y_train_n)
validation_set = TensorDataset(X_val_in, X_val_hr_aux, y_val)
...
for epoch in range(EPOCHS):
    ...
    for Xn, X_hr_aux, yn in train_dataloader:
        Xn = Xn.to(device)
    ...
    with torch.no_grad():
        for Xn, X_hr_aux, y_raw in val_dataloader:
            Xn = Xn.to(device)
            X_hr_aux = X_hr_aux.to(device)
        ...
        torch.save(
            {
                ...
                "training_hparams": {
                    ...
                    "input_channels_description": (
                        "main input channel0=normalized_LR_GCM; "
                        "HR auxiliary channel0=HR_land_sea_mask; "
                        "HR auxiliary channel1=HR_elevation"
                    ),
                    "rcm_var": rcm_var,
                    "gcm_name": gcm_name,
                    "rcm_name": rcm_name,
