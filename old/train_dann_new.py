...
DOMAIN_CLASSIFIER_NAME = "conditional_vgg"
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Train prediction only for this many epochs before DANN loss starts.",
    )
    parser.add_argument(
        "--domain_condition",
        type=str,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional previous-run normalized validation MSE for the same "
            "validation years/metric. Used only for log comparison."
        ),
    )
    parser.add_argument(
        "--baseline_name",
        type=str,
        default="baseline",
        help="Label for --baseline_val_loss in logs.",
    )
    ...
    if args.domain_warmup_epochs < 0:
        raise ValueError("domain_warmup_epochs must be >= 0.")
...
def make_save_root(args, exp_folder_name):
    if args.save_dir is not None:
        ...
        save_root = os.path.join(
            ...
            "trained_models",
            MODEL_NAME,
            "optuna_lr_gcm_with_hr_mask_elevation_to_hr_rcm_conditional_vgg_dann",
        )
...
def train_one_trial(trial, args, data, save_root, device):
    ...
    epochs_no_improve = 0
    domain_train_epochs = max(1, args.epochs - args.domain_warmup_epochs)
    total_domain_steps = max(1, domain_train_epochs * len(source_loader))
    domain_step = 0
    ...
    try:
        for epoch in range(args.epochs):
            ...
            use_domain = epoch >= args.domain_warmup_epochs
            ...
            for Xs, aux_s, ys, condition_s in source_loader:
                ...
                if use_domain:
                    ...
                    progress = domain_step / total_domain_steps
                ...
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    if use_domain:
                        ...
                        source_labels = None
                        target_labels = None
                        domain_loss = sr_loss.new_tensor(0.0)
    ...
    if args.baseline_val_loss is not None:
        delta = best_val_loss - args.baseline_val_loss
        trial.set_user_attr("baseline_name", args.baseline_name)
        trial.set_user_attr("baseline_val_loss", args.baseline_val_loss)
        trial.set_user_attr("delta_vs_baseline_val_loss", delta)
        print(
            f"Trial {trial.number:04d} comparable metric | "
            f"normalized full-image target val MSE={best_val_loss:.10f} | "
            f"{args.baseline_name}={args.baseline_val_loss:.10f} | "
            f"delta={delta:+.10f}"
        )
    else:
        print(
...
def build_config(args, hparams):
    return {
        ...
        "domain_loss": (
            ...
            "gradient reversal and a VGG-style domain classifier"
        ),
        "domain_classifier": DOMAIN_CLASSIFIER_NAME,
        "domain_condition": args.domain_condition,
        "domain_condition_dim": domain_condition_dim(args),
        "domain_warmup_epochs": args.domain_warmup_epochs,
        "target_domain": (
        ...
        "comparison_metric": (
            "normalized full-image target validation MSE over the full selected "
            "validation split"
        ),
...
def main():
    ...
    print("DANN domain classifier:", DOMAIN_CLASSIFIER_NAME)
    print("DANN domain condition:", args.domain_condition)
    print("DANN domain warmup epochs:", args.domain_warmup_epochs)
    print(
        "Comparison metric:",
        "normalized full-image target validation MSE over the full selected validation split",
    )
    if args.baseline_val_loss is not None:
        print(
            "Comparison baseline:",
        ...
        print("Comparison baseline: not set")
    ...
    best_summary = {
        ...
        "baseline_name": args.baseline_name if args.baseline_val_loss is not None else None,
        "baseline_val_loss": args.baseline_val_loss,
        "delta_vs_baseline_val_loss": (
            best_trial.value - args.baseline_val_loss
            if args.baseline_val_loss is not None and best_trial.value is not None
            else None
        ),
    }
        ...
        all_trials_summary.append(
            {
                ...
                "baseline_name": t.user_attrs.get("baseline_name"),
                "baseline_val_loss": t.user_attrs.get("baseline_val_loss"),
                "delta_vs_baseline_val_loss": t.user_attrs.get(
                    "delta_vs_baseline_val_loss"
                ),
