...
#SBATCH --nodes=1
#SBATCH --time=8:00:00
#SBATCH --job-name=rcan_dann_aux
#SBATCH --mem=64G
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --output=rcan_dann_aux.%j.log
...
RUN_NAME="dann_${VAL_START_YEAR}_${VAL_END_YEAR}_bs${BATCH_SIZE}"
SAVE_DIR="$EXP_DIR/trained_models/$MODEL_NAME/$RUN_NAME"

python -u /home/wang.shuoc/downscaling/train_dann.py \
    --rcm_var "$RCM_VAR" \
    --gcm_name "$GCM_NAME" \
    --rcm_name "$RCM_NAME" \
    --grid "$GRID" \
